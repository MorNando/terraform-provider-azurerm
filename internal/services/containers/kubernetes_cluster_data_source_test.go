...
var kubernetesDataSourceTests = map[string]func(t *testing.T){
	...
	"nodeTaints":                                       testAccDataSourceKubernetesCluster_nodeTaints,
...
func TestAccDataSourceKubernetesCluster_nodeTaints(t *testing.T) {
	checkIfShouldRunTestsIndividually(t)
	testAccDataSourceKubernetesCluster_nodeTaints(t)
}
	...
	data.DataSourceTest(t, []acceptance.TestStep{
		{
			...
			Check: acceptance.ComposeTestCheckFunc(
				check.That(data.ResourceName).Key("agent_pool_profile.0.node_taints.0").HasValue("CriticalAddonsOnly=true:NoSchedule"),
			),
		},
	})
}
...
func (KubernetesClusterDataSource) nodeTaintsConfig(data acceptance.TestData) string {
	return fmt.Sprintf(`
%s
...
`, KubernetesClusterResource{}.nodeTaintsConfig(data))
}
