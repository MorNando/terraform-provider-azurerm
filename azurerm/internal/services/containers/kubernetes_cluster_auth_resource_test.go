...
var kubernetesAuthTests = map[string]func(t *testing.T){
	...
	"enablePodSecurityPolicy":        testAccKubernetesCluster_enablePodSecurityPolicy,
...
func TestAccKubernetesCluster_enablePodSecurityPolicy(t *testing.T) {
	checkIfShouldRunTestsIndividually(t)
	testAccKubernetesCluster_enablePodSecurityPolicy(t)
}
...
func testAccKubernetesCluster_enablePodSecurityPolicy(t *testing.T) {
	...
	data.ResourceTest(t, r, []resource.TestStep{
		{
			Config: r.enablePodSecurityPolicyConfig(data),
			Check: resource.ComposeTestCheckFunc(
				check.That(data.ResourceName).ExistsInAzure(r),

				check.That(data.ResourceName).Key("enable_pod_security_policy").HasValue("true"),
			),
		},
		data.ImportStep(),
	})
...
func (KubernetesClusterResource) enablePodSecurityPolicyConfig(data acceptance.TestData) string {
	return fmt.Sprintf(`
provider "azurerm" {
  features {}
}
