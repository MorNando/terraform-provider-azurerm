...
var kubernetesOtherTests = map[string]func(t *testing.T){
	...
	"nodeTaints":                        testAccKubernetesCluster_nodeTaints,
...
func TestAccKubernetesCluster_nodeTaints(t *testing.T) {
	checkIfShouldRunTestsIndividually(t)
	testAccKubernetesCluster_nodeTaints(t)
}
	...
	data.ResourceTest(t, r, []acceptance.TestStep{
		{
			...
			Check: acceptance.ComposeTestCheckFunc(
				check.That(data.ResourceName).ExistsInAzure(r),
				check.That(data.ResourceName).Key("default_node_pool.0.node_taints.0").HasValue("CriticalAddonsOnly=true:NoSchedule"),
			),
		},
		data.ImportStep(),
	})
...
func (KubernetesClusterResource) nodeTaintsConfig(data acceptance.TestData) string {
	return fmt.Sprintf(`
provider "azurerm" {
  features {}
}
